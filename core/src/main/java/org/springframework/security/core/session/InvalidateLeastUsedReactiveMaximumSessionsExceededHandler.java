...
public final class InvalidateLeastUsedReactiveMaximumSessionsExceededHandler
	...
	public Mono<Void> handle(MaximumSessionsContext context) {
		List<ReactiveSessionInformation> sessions = new ArrayList<>(context.getRegisteredSessions());
		...
		return Flux.fromIterable(leastRecentlyUsedSessionsToInvalidate)
			.doOnComplete(() -> this.logger
				.trace(LogMessage.format("Invalidated %d least recently used sessions for principal %s",
						leastRecentlyUsedSessionsToInvalidate.size(), context.getCurrentSession().getPrincipal())))
